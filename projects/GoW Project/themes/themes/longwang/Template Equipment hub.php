<?php /* Template Name: Template Equipment hub */
get_header();
?>
...
<main class="page parallel-page">
    ...
    <section class="parallel-page__variants variants-parallel variants-parallel--equipment">
...
<?php
$link_field = isset($varianty_section_item['ssylka']) ? $varianty_section_item['ssylka'] : '';
$text = !empty($varianty_section_item['tekst']) ? $varianty_section_item['tekst'] : '';

$link_url = '';
$link_target = '';
$link_title = wp_strip_all_tags($text);

if (is_array($link_field)) {
    $link_url = !empty($link_field['url']) ? $link_field['url'] : '';
    $link_target = !empty($link_field['target']) ? $link_field['target'] : '';
    if (!empty($link_field['title'])) {
        $link_title = $link_field['title'];
    }
} elseif (is_string($link_field)) {
    $link_url = $link_field;
}
...
<?php if (!empty($kak_section['zagolovok']) || !empty($kak_section['punkty'])) { ?>
