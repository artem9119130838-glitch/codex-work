...
- [x] Ликвидация «ловушки вежливости» и глубокий аудит архивных чатов:
  * Внедрен архитектурный барьер Zero-Prose Guard в `AGENTS.md`, `GRAVITY_CONTROL_CENTER.md` и `session_management/SKILL.md` (категорический запрет светской беседы и текста до вызова инструментов сжатия);
  * Разработан и запущен скрипт глубокого аудита `scripts/audit_archived_transcripts.py` (просканировано 42 730 шагов в 93 сессиях brain за 35 дней, выявлено 130 прямых директив и замечаний пользователя);
  * Выявлены и зафиксированы в системном контракте `AGENTS.md` критические бизнес-правила: B2B-стандарты реанимации (запрет инфантильных фраз «аккуратно уточнить», обязательный префикс `Re: ` с оборудованием и компанией, ротация стилей, кулдаун 21 день), 1C:OData Guard (запрет слепой записи без dry-run, удаление только email при Bounce без удаления контакта, сохранение ответственных), запрет домысливания ТЗ клиентам снабжению Китая;
  * Скрипт аудита зарегистрирован в мастер-каталоге `SCRIPTS_CATALOG.md` (Раздел 8).
...
- [x] Ликвидация сбоев деплоя, парсинга тендеров и восстановления пайплайна Bitrix24:
  * Исправлена синтаксическая ошибка Python в f-строке промпта `app/services/llm_service.py` (экранирование `{{`);
  * В `docker-compose.prod.yml` настроено прямое монтирование `volumes: - ./app:/app/app` для применения правок Михаила за 2 секунды без пересборки;
  * В `Dockerfile` гарантированно запечены библиотеки `python-docx`, `openpyxl`, `pypdf`, устранена ошибка `ModuleNotFoundError: No module named 'docx'`;
  * В воркфлоу n8n в ноде `Notify Empty File1` заменен ошибочный `localhost:8000` на сетевой IP `http://10.10.0.1:8000` (синхронизировано в SQLite базе n8n);
  * Настроен защищенный SSH-ключ доступа к GitHub на VPS и запущен изолированный демон автодеплоя `/root/tender-rag-api/scripts/auto_deploy.sh` с предварительной проверкой `python3 -m py_compile`;
  * Успешно протестирован сквозной боевой прогон: создана сделка в Битрикс24 ID 2026 (`ФНПЦ АО "НПО "Марс"`, 539 308.78 руб.).
